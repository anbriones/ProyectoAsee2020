...
public class CenaFragment extends Fragment implements MyAdapterJson.OnListInteractionListener, AdapterBaseDatos.OnListInteractionListener {
    ...
    public void onListInteraction(String nombre, Integer calorias, Integer cantidad, String unidad) {
        ...
        AppExecutors.getInstance().diskIO().execute(new Runnable() {
            ...
            public void run() {
                ...
                final Integer calories = Comidasdatabase.getInstance(getActivity()).daoAlim().getcaloriastotales("cena", f1, f2);
                if (calories != 0) {
    ...
    private void loadAlimentos() {
        ...
        AppExecutors.getInstance().diskIO().execute(new Runnable() {
            @Override
            public void run() {
                List<Alimento> items = Comidasdatabase.getInstance(getActivity()).daoAlim().getAlldiarias("cena",timeStamp,timeStamp2);
